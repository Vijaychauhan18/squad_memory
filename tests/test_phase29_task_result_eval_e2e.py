...
class Phase29TaskResultEvalE2ETest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_phase29_syncs_suggestions_and_respects_manual_scores(self) -> None:
        ...
        second = subprocess.run(
            [
                ...
                "marketing",
                "--notes",
                "Needed tighter structure and cleaner CTA.",
                "--json",
            ],
            check=True,
            capture_output=True,
        ...
        self.assertEqual(payload["task_result_summary"]["task_outcomes"], 2)
        ...
        self.assertEqual(payload["task_result_summary"]["pending_manual_reviews"], 1)
        self.assertEqual(payload["sync_summary"]["inserted_suggestions"], 1)
        self.assertEqual(payload["sync_summary"]["skipped_manual"], 1)
        ...
        ledger = json.loads((self.phase29_dir / "task_result_eval_ledger.json").read_text())
        self.assertIn("sync_summary", ledger)
        self.assertIn("task_result_report", ledger)
        self.assertEqual(ledger["task_result_report"]["manual_scorecards"], 1)

        report_text = (self.phase29_dir / "task_result_eval_report.md").read_text()
        self.assertIn("Phase 29 Task Result Evaluation", report_text)
        self.assertIn("Pending Manual Reviews", report_text)

        queue_text = (self.phase29_dir / "scorecard_review_queue.md").read_text()
        self.assertIn(str(second_payload["outcome_id"]), queue_text)
