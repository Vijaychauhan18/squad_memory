...
PHASE17_BEGIN = "<!-- phase17:begin -->"
PHASE17_END = "<!-- phase17:end -->"

CHARLES_CONFIG: Dict[str, Any] = {
    "label": "Charles",
    "router": "MEMORY.md",
    "operating_canon": "memory/charles-operating-canon-2026.md",
    "external_canon_filename": "charles-external-source-canon-2026.md",
    "summary_note": "live-external-source-monitor.md",
    "topic": "charles_external_sources",
    "intent": "monitoring, research, creator_examples, platform_changes, social_patterns",
    "role": "charles, current, pinchy, marketing",
    "use_for": "fresh_examples, source_selection, platform_monitoring, creator_research",
    "avoid_for": "broad_campaign_strategy_without_social_need, seo_strategy",
    "canonical_group": "Charles external source canon",
    "bundle_roles": ["Charles", "Current", "Pinchy"],
    "consensus": [
        "Fresh Charles sources matter most for platform shifts, creator patterns, and channel-native execution examples, not for replacing the core Charles operating system.",
        "The strongest social sources keep reinforcing native packaging, engagement discipline, and feedback loops over generic cross-posting.",
    ],
    "tension": [
        "The main tradeoff is speed versus signal quality: fresh platform chatter helps, but chasing every new tactic weakens consistency and brand fit.",
    ],
    "action": "Start with the Charles operating canon for the core system, then use this external canon for fresh creator examples, platform changes, and social execution research.",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase 17 external source ingestion and fusion for Charles")
...
def build_domain_summary_note(results: Sequence[Dict[str, Any]], snapshot_hint: Path) -> str:
    ...
    lines = [
        "---",
        "source: local phase17 charles external source ingest",
        "title: Charles External Source Monitor",
        f"scraped: {datetime.now(timezone.utc).date().isoformat()}",
        "tags: phase17_external_sources, charles, monitoring, freshness",
        f"topic: {CHARLES_CONFIG['topic']}",
        f"intent: {CHARLES_CONFIG['intent']}",
        f"role: {CHARLES_CONFIG['role']}",
        "confidence: medium",
        "canonical: true",
        "canonical_group: Charles live external monitor",
        "use_for: freshness, source_selection, creator_research",
        "avoid_for: final_strategy_without_core_canon",
        "---",
        "",
        "# Charles External Source Monitor",
        "",
        "This note is generated by Phase 17. Use it to spot fresh creator, platform, and social-source activity before opening the Charles external canon or source-specific canons.",
        "",
        "## Health Summary",
...
def build_source_canon_note(source_cfg: Dict[str, Any], result: Dict[str, Any], top_n: int) -> str:
    ...
    lines = [
        ...
        f"scraped: {datetime.now(timezone.utc).date().isoformat()}",
        f"tags: live_source_canon, phase17_external_sources, charles, {source_cfg['slug']}",
        f"topic: {source_cfg.get('topic', CHARLES_CONFIG['topic'])}",
        f"intent: {', '.join(source_cfg.get('intent', []))}",
        f"role: {', '.join(source_cfg.get('roles', []))}",
        f"confidence: {source_cfg.get('confidence', 'medium')}",
        "canonical: true",
        "canonical_group: Charles live source canon",
        "use_for: fresh examples, platform monitoring, creator research",
        "avoid_for: final_strategy_without_core_canon",
        "---",
        "",
        ...
        "",
        "## Best Use",
        source_cfg.get("strength", "Use this source for fresh creator examples and social context."),
        "",
        "## Latest Signals",
    ]
    ...
    lines.extend(
        [
            "",
            "## Agent Checklist",
            "- Use this source for freshness first, then map the signal back into the Charles operating canon.",
            "- Pull one reusable platform pattern, engagement idea, or creator move before using the source in final advice.",
            "- Promote a durable note only if the update changes how the squad should post, engage, or plan channel-native work.",
        ]
    )
    return "\n".join(lines).rstrip() + "\n"
...
def build_external_canon(results: Sequence[Dict[str, Any]]) -> str:
    ...
    lines = [
        build_frontmatter(
            {
                "title": "Charles External Source Canon 2026",
                "topic": CHARLES_CONFIG["topic"],
                "intent": CHARLES_CONFIG["intent"],
                "role": CHARLES_CONFIG["role"],
                "use_for": CHARLES_CONFIG["use_for"],
                "avoid_for": CHARLES_CONFIG["avoid_for"],
                "confidence": "high",
                "canonical": "true",
                "canonical_group": CHARLES_CONFIG["canonical_group"],
                "source": "phase17_external_sources",
                "scraped": datetime.now(timezone.utc).date().isoformat(),
                "tags": "phase17, charles, external sources, canon, squad",
            }
        ).rstrip(),
        "# Charles External Source Canon 2026",
        "",
        "## Core Concept",
        CHARLES_CONFIG["consensus"][0] + " " + CHARLES_CONFIG["consensus"][1],
        "",
        "## Best Sources",
    ]
    ...
    lines.extend(
        [
            "",
            "## Workflow Map",
            "1. Start with `memory/charles-operating-canon-2026.md` for the default social execution system.",
            "2. Open this external canon when the task needs fresh creator examples, recent platform shifts, or source-aware social research.",
        ]
    )
    ...
    lines.extend(
        [
            "",
            "## Team Use",
            "- **Charles**: Use this canon for fresh platform and creator patterns without abandoning the core Charles system.",
            "- **Current**: Use this canon when campaign distribution needs more native social examples and channel-aware execution ideas.",
            "- **Pinchy**: Use this canon when the ask explicitly needs recent social examples, platform changes, or source-backed creator research.",
        ]
    )
    return "\n".join(lines).rstrip() + "\n"


def build_phase17_block(evidence: Dict[str, Any]) -> str:
    lines = [
        "## Phase 17 External Source Fusion",
        "",
...
def replace_phase17_block(body: str, block: str) -> str:
    import re

    managed = f"{PHASE17_BEGIN}\n{block.rstrip()}\n{PHASE17_END}"
    pattern = re.compile(r"\n?<!-- phase17:begin -->.*?<!-- phase17:end -->\n?", re.DOTALL)
...
def domain_evidence(results: Sequence[Dict[str, Any]], canon_rel_path: str) -> Dict[str, Any]:
    ...
    source_signals = [
        {
            "source": result["name"],
            "path": f"charles/memory/live-source-canon-{result['slug']}.md",
    ...
    return {
        ...
        "consensus": CHARLES_CONFIG["consensus"],
        "tension": CHARLES_CONFIG["tension"],
        "squad_action": CHARLES_CONFIG["action"],
...
def build_report(result: Dict[str, Any]) -> str:
    lines = [
        "---",
        "source: local phase17 charles external sources",
        "title: Charles External Sources Report",
        f"scraped: {datetime.now(timezone.utc).date().isoformat()}",
        "tags: phase17_external_sources, charles, canon, monitoring",
        "topic: charles_external_sources",
        "intent: maintenance, monitoring, canon_fusion",
        "role: pinchy, charles, current, marketing",
        "confidence: high",
        "canonical: false",
        "canonical_group: Charles external source report",
        "use_for: source_monitoring, phase17_reporting",
        "avoid_for: seo_strategy",
        "---",
        "",
        "# Charles External Sources Report",
        "",
...
def run(skills_root: Path, config_path: Path, phase17_dir: Path, db_path: Path, top_n: int, build_db: bool) -> Dict[str, Any]:
    ...
    router_path = skills_root / "charles" / CHARLES_CONFIG["router"]
    operating_canon_path = skills_root / "charles" / CHARLES_CONFIG["operating_canon"]
    external_canon_path = memory_root / CHARLES_CONFIG["external_canon_filename"]
    summary_path = memory_root / CHARLES_CONFIG["summary_note"]

    snapshot_dir = phase17_dir / "raw" / "charles"
    state_path = phase17_dir / "state" / "charles.json"
    ...
    router_changed = replace_canonical_notes_block(router_path, CHARLES_CONFIG["external_canon_filename"])
    bundle_changes: List[str] = []
    for role_name in CHARLES_CONFIG["bundle_roles"]:
        if replace_bundle_block(router_path, role_name, CHARLES_CONFIG["external_canon_filename"]):
            bundle_changes.append(role_name)

    external_canon_rel = f"charles/memory/{CHARLES_CONFIG['external_canon_filename']}"
    evidence = domain_evidence(results, external_canon_rel)
    ledger_path = phase17_dir / "charles_external_evidence_ledger.json"
    write_json(
        ledger_path,
        {
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "topics": {CHARLES_CONFIG["topic"]: evidence},
    ...
    operating_canon_changed = write_note_body(
        ...
        lambda body, evidence=evidence: replace_phase17_block(body, build_phase17_block(evidence)),
    )
    ...
    report_path = phase17_dir / "charles_external_sources_report.md"
    ...
    manifest_path = phase17_dir / f"phase17-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.json"
    write_json(manifest_path, result)
    write_json(phase17_dir / "latest.json", result)
...
def main() -> int:
    ...
    result = run(
        ...
        phase17_dir=Path(args.phase17_dir),
