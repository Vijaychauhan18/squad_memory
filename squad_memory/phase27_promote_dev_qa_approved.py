...
DEFAULT_PHASE26 = BASE / "ingest" / "phase26" / "latest.json"
DEFAULT_PHASE27_DIR = BASE / "ingest" / "phase27"
DEFAULT_DECISIONS = DEFAULT_PHASE27_DIR / "decisions.json"
DEFAULT_STATE = DEFAULT_PHASE27_DIR / "state.json"
...
MEMORY_BEGIN = "<!-- BEGIN AUTO APPROVED DEV QA PROMOTIONS -->"
MEMORY_END = "<!-- END AUTO APPROVED DEV QA PROMOTIONS -->"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase 27 reviewed promotion of approved developer and QA drafts")
    parser.add_argument("--phase26-manifest", default=str(DEFAULT_PHASE26))
...
def sync_decisions(
    ...
    tracked = phase26_manifest.get("tracked_candidates", [])
        ...
        synced.append(
            {
                ...
                "draft_topic": candidate.get("source_topic", prior.get("draft_topic", "")),
    ...
    payload = {
        "generated_at": now,
        "phase26_manifest": phase26_manifest.get("manifest_path") or phase26_manifest.get("phase26_dir", ""),
...
def build_promoted_note(draft_path: Path, decision: Dict[str, Any], approved_at: str) -> str:
    ...
    topic = meta.get("topic", decision.get("draft_topic", f"{domain}_external_signal"))
        ...
        build_frontmatter(
            {
                ...
                "canonical": "false",
                "canonical_group": f"{label} approved external promotions",
                "use_for": use_for,
                "avoid_for": "blind_reuse_without_source_check",
                "promoted_from": f"phase26/{draft_path.name}",
    ...
    for item in draft["why_queue"] or ["Approved after Phase 26 review."]:
        lines.append(f"- {item}")
...
def refresh_router_blocks(skills_root: Path, approved_records: Sequence[Dict[str, Any]]) -> None:
    for domain in DOMAIN_LABELS:
        ...
        if records or (MEMORY_BEGIN in text and MEMORY_END in text):
            text = insert_or_replace(text, MEMORY_BEGIN, MEMORY_END, memory_block(domain, records), "")
...
def write_review_note(
    ...
    lines = [
        "---",
        "source: local phase27 promotion review",
        "title: Developer QA Promotion Review",
        f"scraped: {datetime.now(timezone.utc).date().isoformat()}",
        "tags: phase27_promote, promotion_review, approval_gate, developer, qa",
        "topic: developer_qa_durable_promotion",
        "intent: review, approval, durable_promotion",
        "role: pinchy, developer, qa, reviewer, devops",
        "confidence: medium",
        "canonical: false",
        "canonical_group: Developer QA promotion review",
        "use_for: approval_state, promoted_notes, review_summary",
        "avoid_for: direct_strategy_without_note_review",
        "---",
        "",
        "# Developer QA Promotion Review",
        "",
...
def promote_approved(
    ...
    decisions = sync_decisions(phase26_manifest, decisions_path, approve, reject, hold)
    state = load_json(state_path, {"promoted_drafts": {}})
    promoted_drafts: Dict[str, Dict[str, Any]] = state.get("promoted_drafts", {})

    drafts_dir = Path(phase26_manifest.get("phase26_dir", phase27_dir.parent / "phase26")) / "drafts"
    ...
    result = {
        ...
        "phase26_manifest": phase26_manifest.get("manifest_path", str(DEFAULT_PHASE26)),
        ...
        "review_note": str(write_review_note(phase27_dir / "review-status.md", decisions, approved_records, promoted_now, skipped, conflicts)),
    }
    if build_result:
        result["build"] = build_result

    manifest_path = phase27_dir / f"phase27-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.json"
    write_json(manifest_path, result)
    write_json(phase27_dir / "latest.json", result)
...
def main() -> int:
    args = parse_args()
    phase26_manifest = load_json(Path(args.phase26_manifest), {})
    if not phase26_manifest:
        raise SystemExit(f"Missing Phase 26 manifest: {args.phase26_manifest}")
    result = promote_approved(
        phase26_manifest=phase26_manifest,
        phase27_dir=Path(args.phase27_dir),
